...
class Helper
{
    ...
    public static function getHeaderCategory()
    {
        ...
        if ($menu) {
            ...
            <li>
                <a href="product-grids">Sản phẩm<i class="ti-angle-down"></i></a>
    ...
    public static function getCartId($user_id = ''): string
    {
        if (Auth::check()) {
            if ($user_id == "") $user_id = auth()->user()->id;
            return Cart::where('user_id', $user_id)->where('order_id', null)->pluck('id');
        }
    ...
    public static function wishlistCount($user_id = '')
    {
        if (Auth::check()) {
            ...
            $countItems = Wishlist::where('user_id', $user_id)->where('cart_id', null)->sum('quantity');
            if ($countItems == 1) {
