    ...
    <div class="card-body">
      ...
      <div class="form-group {{(($category->is_parent==1) ? 'd-none' : '')}}" id='parent_cat_div'>
        ...
        <div class="form-group">
          ...
          <input id="thumbnail" class="form-control" type="text" name="photo" value="{{asset(str_replace('http://localhost', '', $category->photo))}}">
        </div>
        <div id="holder" style="margin-top:15px;max-height:200px;">
            @if(!empty($category->photo))
                <img style="margin-top:10px;max-height:200px;" src="{{asset(str_replace('http://localhost', '', $category->photo))}}">
            @endif
        </div>
          @error('photo')
          <span class="text-danger">{{$message}}</span>
